...
public class TripDivisionInfoController {
    ...
    @Autowired
		...
		public ResponseEntity<TripDivisionInfo> getTripDivisionById(@PathVariable long division_ID){
		 ...
		 @PostMapping("/tripdiv")
		 public ResponseEntity<TripDivisionInfo> saveTripDivisionInfo(@RequestBody TripDivisionInfo tripDivInfo){
		 	TripDivisionInfo tripDivAdded = tripDivisionInfoService.saveTripDivisionInfo(tripDivInfo);
		 	return new ResponseEntity<TripDivisionInfo>(tripDivAdded, HttpStatus.CREATED);
		 }
